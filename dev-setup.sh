...
if ! command -v picoclaw &>/dev/null; then
  ...
  if [ -z "$LATEST_URL" ]; then
    ...
    sudo cp /tmp/picoclaw-src/bin/picoclaw /usr/local/bin/picoclaw
    cd -
  else
...
mkdir -p ~/.picoclaw/skills
cp ./picoclaw/skills/farm_control.sh ~/.picoclaw/skills/
chmod +x ~/.picoclaw/skills/*.sh
echo "✓ Config PicoClaw copiée"
